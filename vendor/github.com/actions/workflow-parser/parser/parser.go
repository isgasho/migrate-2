...
func (p *Parser) parseOn(workflow *model.Workflow, node ast.Node) {
	...
	if IsSchedule(strVal) {
		workflow.On = &model.OnSchedule{Expression: extractScheduleExpression(strVal)}
		return
	}
...
func (p *Parser) parseUses(action *model.Action, node ast.Node) {
	...
	uses, err := ParseUsesString(strVal, action.Identifier)
	action.Uses = uses
	if err != nil {
		p.addError(node, err.Error())
	}
}

func ParseUsesString(strVal string, identifer string) (model.Uses, error) {
	if strVal == "" {
		return &model.UsesInvalid{}, errors.Errorf("`uses' value in action `%s' cannot be blank", identifer)
	}
	if strings.HasPrefix(strVal, "./") {
		return &model.UsesPath{Path: strings.TrimPrefix(strVal, "./")}, nil
	}

	if strings.HasPrefix(strVal, "docker://") {
		return &model.UsesDockerImage{Image: strings.TrimPrefix(strVal, "docker://")}, nil
	}

	tok := strings.Split(strVal, "@")
	if len(tok) != 2 {
		return &model.UsesInvalid{Raw: strVal}, errors.New("The `uses' attribute must be a path, a Docker image, or owner/repo@ref")
	}
	ref := tok[1]
